...
void* http_req(void* data)
{
	...
	*/
	...
	puts("Document ROOT dfdf12");	
	th=*((int *)data); //Type casting void pointer to integer pointer
	int a=0;
	//int rc = read(th, buf, sizeof(buf));
	char buf1[1000];
//	r=http_readline(th,buf,sizeof(buf));
	puts("Document ROOT 7778555");
	r=read(th,buf,sizeof(buf));
	puts("Document ROOT 888888555");
	puts(buf);
 	//r=http_readline(th,buf,sizeof(buf));// 
	
	puts("Document ROOT 12");
 	...
 	if((strcmp(tmp_var,"GET"))==0);// Comparing the temporary array to check the string
	 	{
	 	 ...
	 	 // filePath is the name of the file to map the URL tmp_ptr1 and 2 are used to perform string operation
		        ...
		        strcpy(tmp_ptr3,tmp_ptr2);//coping tmp_ptr2 value to tmp_ptr3
		       	tmp_ptr5=strrchr(tmp_ptr3,'/');       
			
			/*V2 - Store requested file names in the cache struct*/
			char compare_file[20], temp_compare_file_store[20], temp_str[20];;
			bzero(compare_file, sizeof(compare_file));
			bzero(temp_compare_file_store, sizeof(temp_compare_file_store));
			bzero(temp_str, sizeof(temp_str));
			sprintf(compare_file, "%s", ".");
			
			
			printf("\n\nTHE REQUEST IS %s", buf);

			/*Look till proxy and then return the next path*/
			int t;
			char *scan_buf;
			scan_buf = strstr(buf, "proxy");
			printf("\n\nAFTER strstr %s", scan_buf);
			
			/*Don't take tmp_ptr5 for folders*/
			memcpy(&temp_compare_file_store[0], scan_buf, strlen(scan_buf));
			
//			memcpy(&temp_compare_file_store[0], scan_buf, strlen(scan_buf));
			memcpy(&temp_str[0], &temp_compare_file_store[5], (strlen(temp_compare_file_store)-5));
			printf("\n\nAFTER REMOVING THE WORK PROXY %s", temp_str);
			
			/*Appending dot*/
			memcpy(&compare_file[1], temp_str, strlen(temp_str));
			
			printf("\n\nThe FILE NAME IS ---->!@#: %s", compare_file);
			strcpy(cache_files[cache_file_count].filename, compare_file);
			
			printf("\n\n------>The name stored is %s\n\n", cache_files[cache_file_count].filename);
			cache_file_count++;
	 	...
	 	else
	 	{
		       ...
		       // printf("\n %s \n\n\t",filePath);
		        ...
		        strcpy(filePath,strcat(doc_root,temp_str));//Adding the requested file.
...
/*=========================== Main function- Start ===========================*/
/*V2 - creating another thread to update the tables*/
int main(int argc, char *argv[])
{

	int fd,nfd,th_id,portnum, sync_socket;// fd will be used as a socket descriptor
	...
	/*V2 - Creating another thread*/
	pthread_create(&p_thread[i+2],&attr_t, (void *) scan_periodically, (void*)&nfd);
